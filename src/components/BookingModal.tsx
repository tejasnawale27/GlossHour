...
import { saveBooking, type BookingRecord } from "../data/bookings";
import BoardingPassModal from "./BoardingPassModal";

interface BookingModalProps {
  salon: Salon | null;
  onClose: () => void;
  onConfirmed: (booking: BookingRecord) => void;
}
...
export default function BookingModal({
  ...
  const toggleService = (svcName: string) =>
    setSelectedServices((prev) => {
      ...
      next.has(svcName) ? next.delete(svcName) : next.add(svcName);
      return next;
    });
  ...
  const handleConfirm = () => {
    ...
    const record: BookingRecord = {
      ...
      date: selectedDate,
      dateLabel: dateObj?.label ?? selectedDate,
      time: selectedSlot,
  ...
  const handleAddToCalendar = () => {
    ...
    const url =
      ...
      `&details=${encodeURIComponent(`Booking ref: ${booking.id}\nServices: ${booking.services.map((s) => s.name).join(", ")}`)}` +
  ...
  const animClass = direction === "forward" ? "animate-step-in" : "animate-step-back";
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center">
      <div
        ...
        onClick={isStep4 ? undefined : onClose}
        aria-hidden
      />

      <div
        className={`relative w-full flex flex-col bg-slate-900 border border-white/8 rounded-t-3xl sm:rounded-3xl shadow-2xl shadow-black/70 animate-scale-in transition-all duration-500 ${
          isStep4
            ? "sm:max-w-md max-h-[96dvh] sm:max-h-[92vh]"
            : "sm:max-w-xl max-h-[96dvh] sm:max-h-[90vh]"
        }`}
      >
        {/* ── Hero banner (hidden on step 4) ── */}
        {!isStep4 && (
          <div className="relative h-36 sm:h-44 shrink-0 overflow-hidden rounded-t-3xl">
            ...
            <div className="absolute bottom-0 left-0 right-0 px-5 pb-4">
              ...
              <div className="flex items-end justify-between gap-2">
                <h2 className="font-display text-xl sm:text-2xl text-white leading-tight">{salon.name}</h2>
                <div className="shrink-0 inline-flex items-center gap-1 pb-0.5">
                  <Star className="w-3.5 h-3.5 text-amber-400 fill-amber-400" />
                  <span className="text-sm font-semibold text-white">{salon.rating.toFixed(1)}</span>
                </div>
              </div>
            </div>
          </div>
              ...
              {STEP_META.map((meta, i) => {
                ...
                return (
                  <div key={n} className="flex items-center gap-2 flex-1 min-w-0">
                    ...
                    >
                      {done ? <CheckCircle2 className="w-4 h-4" strokeWidth={2.5} /> : <Icon className="w-3.5 h-3.5" strokeWidth={2} />}
                    </button>
                    <span className={`text-xs font-medium transition-colors truncate ${active ? "text-amber-400" : done ? "text-slate-300" : "text-slate-500"}`}>
                      {meta.label}
                    </span>
                    {i < STEP_META.length - 1 && (
                      <div className={`flex-1 h-px mx-1 transition-all ${step > n ? "bg-amber-400/50" : "bg-white/8"}`} />
                    )}
                  </div>
                );
              })}
        ...
        <div className="flex-1 overflow-y-auto no-scrollbar">
          <div key={step} className={`${animClass} ${isStep4 ? "px-0 py-0" : "px-5 py-5"}`}>

            {/* ═══ STEP 1 ═══ */}
            {step === 1 && (
              <div>
                <p className="text-xs uppercase tracking-[0.15em] text-slate-400 mb-3">Select one or more services</p>
                <div className="space-y-2">
                  {salon.services.map((s) => {
                    ...
                    return (
                      ...
                      >
                        <div className={`shrink-0 w-5 h-5 rounded-md border flex items-center justify-center transition-all ${checked ? "bg-amber-400 border-amber-400" : "bg-white/5 border-white/15"}`}>
                          {checked && (
                            <svg viewBox="0 0 10 8" className="w-3 h-2.5" fill="none">
                              <path d="M1 4l3 3 5-6" stroke="#0a0a14" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" />
                            </svg>
                          )}
                        </div>
                        <div className="flex-1 min-w-0">
                          <div className={`text-sm font-medium truncate transition-colors ${checked ? "text-white" : "text-slate-200"}`}>{s.name}</div>
            ...
            {step === 2 && (
              <div className="space-y-6">
                <div>
                  <p className="text-xs uppercase tracking-[0.15em] text-slate-400 mb-3">Choose a date</p>
                      ...
                      return (
                        <button
                          ...
                          className={`flex flex-col items-center py-3 rounded-2xl border transition-all duration-200 ${
                            active ? "border-amber-400/50 bg-amber-400/[0.08]" : "border-white/6 bg-white/[0.025] hover:bg-white/[0.05]"
                          }`}
                        >
                          <span className={`text-[10px] uppercase tracking-wider ${active ? "text-amber-400" : "text-slate-500"}`}>{d.day}</span>
                          <span className={`text-lg font-bold mt-0.5 tabular-nums ${active ? "text-amber-400" : "text-white"}`}>{d.date}</span>
                          <span className="text-[10px] text-slate-500 mt-0.5">{d.month}</span>
                        </button>
                      );
                    })}
                  </div>
                ...
                <div>
                  <p className="text-xs uppercase tracking-[0.15em] text-slate-400 mb-3">Pick a time slot</p>
                            ...
                            return (
                              <button
                                ...
                                className={`px-3 py-1.5 rounded-lg text-xs font-medium border transition-all duration-200 ${
                                  active ? "border-amber-400/50 bg-amber-400/[0.08] text-amber-400" : "border-white/6 bg-white/[0.025] text-slate-300 hover:bg-white/[0.06] hover:text-white"
                                }`}
                              >
                                {slot}
                              </button>
              ...
              <div className="space-y-5">
                <div className="bg-white/[0.03] border border-white/8 rounded-2xl overflow-hidden">
                  <div className="px-4 py-3 border-b border-white/5">
                    <p className="text-xs uppercase tracking-[0.15em] text-slate-400">Booking summary</p>
                  </div>
                  <div className="px-4 py-3 space-y-2">
                    ...
                    <div className="flex justify-between gap-3 text-sm">
                      ...
                      <span className="text-white font-medium">
                        {dateObj ? `${dateObj.day}, ${dateObj.date} ${dateObj.month}` : "—"} · {selectedSlot}
                      </span>
                    </div>
                    <div className="flex justify-between gap-3 text-sm">
                    ...
                    <div className="pt-1 border-t border-white/5">
                      {chosenServices.map((s) => (
                        <div key={s.name} className="flex justify-between gap-3 text-sm py-1">
                          <span className="text-slate-300 truncate">{s.name}</span>
                          <span className="text-slate-300 shrink-0 tabular-nums">{formatINR(s.price)}</span>
                        </div>
                      ))}
                    </div>
                    <div className="flex justify-between gap-3 pt-2 border-t border-white/5">
                      <span className="text-sm font-semibold text-white">Total</span>
                      <span className="text-base font-bold text-amber-400 tabular-nums">{formatINR(cartTotal)}</span>
                    </div>
                  </div>
                </div>

                <div>
                  <label htmlFor={`${uid}-name`} className="block text-xs font-medium text-slate-300 mb-1.5">Full name</label>
                  ...
                  {nameErr && <p className="text-xs text-rose-400 mt-1">{nameErr}</p>}
                </div>

                <div>
                  <label htmlFor={`${uid}-phone`} className="block text-xs font-medium text-slate-300 mb-1.5">Mobile number</label>
                  <div className="relative">
                    ...
                    <input
                      ...
                      maxLength={10}
                      value={phone}
                      onChange={(e) => { setPhone(e.target.value.replace(/\D/g, "")); if (phoneErr) setPhoneErr(""); }}
                  ...
                  {phoneErr && <p className="text-xs text-rose-400 mt-1">{phoneErr}</p>}
                </div>
          ...
          <div className="shrink-0 px-5 pb-6 pt-3 border-t border-white/5 bg-slate-900">
            {step === 1 && (
              <div className="flex items-center justify-between gap-3 mb-3 px-4 py-2.5 bg-white/[0.03] border border-white/6 rounded-xl">
                ...
                <div className="text-sm font-semibold text-amber-400 tabular-nums">{formatINR(cartTotal)}</div>
              </div>
            )}
